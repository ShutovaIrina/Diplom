...
Функция СоздатьДокументНаСервере(МесяцНачисления) Экспорт
	...
	Пока ВыборкаДоговор.Следующий() Цикл
		...
		КонецЕсли;
		ДокРеализацииОбъект.ВыполнитьАвтозаполнение();     
		ДокРеализацииОбъект.ПроверитьЗаполнение();
		ДокРеализацииОбъект.Записать(РежимЗаписиДокумента.Проведение);
		ДокРеализацииСсылка = ДокРеализацииОбъект.Ссылка;
		МассивДоговоров.Вставить(n, ВыборкаДоговор.Договор);
		МассивДокументов.Вставить(n, ДокРеализацииСсылка);
		n=n+1;
	КонецЦикла;	
